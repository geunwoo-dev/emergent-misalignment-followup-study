...
def emit_eval_loop_script(root: Path, persona_root: Path, config_glob: str) -> str:
    return "\n".join(
        [
            ...
            '  python3 "$ROOT/followup_study/multi_judge_eval.py" --eval_config "$config"',
            "done",
            "",
        ]
    )
...
def emit_calibration_script(root: Path, persona_root: Path, calibration: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            'cd "$ROOT"',
            "",
            'JUDGE_CONFIGS=("$ROOT"/followup_study/generated/judge_configs/*.json)',
            'python3 "$ROOT/followup_study/evaluate_judge_calibration.py" \\',
            f'  --persona_root "{persona_root}" \\',
            f'  --calibration_set "$ROOT/{calibration["calibration_set_path"]}" \\',
            '  --judge_config_paths "${JUDGE_CONFIGS[@]}" \\',
            f"  --dimensions {dims} \\",
            f'  --output_dir "$ROOT/followup_study/generated/calibration_reports" \\',
...
def emit_checkpoint_eval_script(root: Path, persona_root: Path, traits: list[str]) -> str:
    ...
    return "\n".join(
        [
            ...
            '    python3 "$ROOT/followup_study/multi_judge_eval.py" \\',
            '      --repo_root "$ROOT" \\',
            ...
            '      --max_tokens 1000 \\',
            '      --judge_config_paths "${JUDGE_CONFIGS[@]}" \\',
            '      --raw_output_path "$ROOT/followup_study/generated/raw_outputs/checkpoints/$RUN_SLUG/$checkpoint_label/$trait.csv" \\',
            '      --judge_output_dir "$ROOT/followup_study/generated/judge_outputs/checkpoints/$RUN_SLUG/$checkpoint_label/$trait" \\',
            '      --merged_output_path "$ROOT/followup_study/generated/merged_outputs/checkpoints/$RUN_SLUG/$checkpoint_label/$trait.csv" \\',
            '      --agreement_report_path "$ROOT/followup_study/generated/agreement_reports/checkpoints/$RUN_SLUG/$checkpoint_label/$trait.json" \\',
            '      --phase checkpoints \\',
            '      --run_label "$RUN_SLUG" \\',
...
def emit_critical_point_script(root: Path, spec: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            'REPORTS_ROOT="${REPORTS_ROOT:-$ROOT/followup_study/generated/agreement_reports/checkpoints/$RUN_LABEL}"',
            'CURVE_CSV="$ROOT/followup_study/generated/critical_point_curves/${RUN_LABEL}__${TRAIT}.csv"',
            'REPORT_JSON="$ROOT/followup_study/generated/critical_point_reports/${RUN_LABEL}__${TRAIT}.json"',
            'cd "$ROOT"',
            "",
            'python3 "$ROOT/followup_study/collect_trait_curves.py" \\',
            '  --reports_root "$REPORTS_ROOT" \\',
            '  --run_label "$RUN_LABEL" \\',
            '  --trait "$TRAIT" \\',
            '  --output_csv "$CURVE_CSV"',
            "",
            'python3 "$ROOT/followup_study/detect_critical_points.py" \\',
            '  --curve_csv "$CURVE_CSV" \\',
...
def emit_sae_export_script(root: Path, generated_root: Path, train_runs: list[dict], datasets: list[dict], sae_defaults: dict) -> str:
    ...
    for run in train_runs:
        ...
        for split_name in [dataset["control_level"], run["level"]]:
            ...
            dataset_path = Path("prev_paper_materials/persona/dataset") / run["dataset"] / f"{split_name}.jsonl"
            lines.extend(
                [
                    'CUDA_VISIBLE_DEVICES="$GPU" python3 "followup_study/export_activations.py" \\',
...
def emit_train_sae_script(root: Path, generated_root: Path, train_runs: list[dict], sae_defaults: dict) -> str:
    ...
    for run in train_runs:
        ...
        lines.extend(
            [
                'python3 "followup_study/train_sae.py" \\',
...
def emit_score_sae_script(root: Path, generated_root: Path, train_runs: list[dict], sae_defaults: dict) -> str:
    ...
    for run in train_runs:
        ...
        lines.extend(
            [
                'python3 "followup_study/score_sae_features.py" \\',
...
def emit_early_stop_script(root: Path, spec: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            'cd "$ROOT"',
            "",
            'RUN_LABEL="$RUN_SLUG" TRAIT="$TRAIT" bash "$ROOT/followup_study/generated/runbooks/55_detect_critical_points.sh"',
            'EARLY_STOP_MODEL=$(python3 "$ROOT/followup_study/select_early_stop.py" --critical_point_report "$ROOT/followup_study/generated/critical_point_reports/${RUN_SLUG}__${TRAIT}.json" --run_output_dir "$RUN_OUTPUT_DIR" --print_path)',
            ...
            '    --max_tokens 1000 \\',
            '    --judge_config_paths "${JUDGE_CONFIGS[@]}" \\',
            '    --raw_output_path "$ROOT/followup_study/generated/raw_outputs/interventions/early_stop/$RUN_SLUG/$eval_trait.csv" \\',
            '    --judge_output_dir "$ROOT/followup_study/generated/judge_outputs/interventions/early_stop/$RUN_SLUG/$eval_trait" \\',
            '    --merged_output_path "$ROOT/followup_study/generated/merged_outputs/interventions/early_stop/$RUN_SLUG/$eval_trait.csv" \\',
            '    --agreement_report_path "$ROOT/followup_study/generated/agreement_reports/interventions/early_stop/$RUN_SLUG/$eval_trait.json" \\',
            ...
            '    --max_tokens 1000 \\',
            '    --judge_config_paths "${JUDGE_CONFIGS[@]}" \\',
            '    --raw_output_path "$ROOT/followup_study/generated/raw_outputs/interventions/full_train/$RUN_SLUG/$eval_trait.csv" \\',
            '    --judge_output_dir "$ROOT/followup_study/generated/judge_outputs/interventions/full_train/$RUN_SLUG/$eval_trait" \\',
            '    --merged_output_path "$ROOT/followup_study/generated/merged_outputs/interventions/full_train/$RUN_SLUG/$eval_trait.csv" \\',
            '    --agreement_report_path "$ROOT/followup_study/generated/agreement_reports/interventions/full_train/$RUN_SLUG/$eval_trait.json" \\',
            '    --phase intervention_full_train \\',
            '    --run_label "$RUN_SLUG" \\',
            '    --model_alias "$MODEL_ALIAS"',
            'done',
            "",
            'python3 "$ROOT/followup_study/evaluate_intervention.py" \\',
            '  --baseline_report_dir "$ROOT/followup_study/generated/agreement_reports/interventions/full_train/$RUN_SLUG" \\',
            '  --intervention_report_dir "$ROOT/followup_study/generated/agreement_reports/interventions/early_stop/$RUN_SLUG" \\',
            '  --output_csv "$ROOT/followup_study/generated/intervention_reports/${RUN_SLUG}__early_stop.csv"',
            "",
        ]
    )
...
def emit_filter_data_script(root: Path, spec: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            'TMP_CSV="$ROOT/followup_study/generated/tmp/dataset_filter/raw.csv"',
            'TMP_DIR="$ROOT/followup_study/generated/tmp/dataset_filter/judged"',
            'MERGED_CSV="$ROOT/followup_study/generated/tmp/dataset_filter/merged.csv"',
            'JUDGE_CONFIGS=("$ROOT"/followup_study/generated/judge_configs/*.json)',
            'cd "$ROOT"',
            "",
            'python3 "$ROOT/followup_study/dataset_to_response_csv.py" --dataset_path "$DATASET_PATH" --output_csv "$TMP_CSV"',
            'for judge_config in "${JUDGE_CONFIGS[@]}"; do',
            '  judge_name=$(basename "$judge_config" .json)',
            '  python3 "$ROOT/followup_study/judge_saved_outputs.py" \\',
            '    --persona_root "$PERSONA_ROOT" \\',
            '    --input_csv "$TMP_CSV" \\',
            '    --trait "$TRAIT" \\',
            ...
            'done',
            'python3 "$ROOT/followup_study/aggregate_multi_judge.py" \\',
            '  --trait "$TRAIT" \\',
            '  --input_paths "$TMP_DIR"/*.csv \\',
            '  --output_merged_csv "$MERGED_CSV" \\',
            '  --output_summary_json "$ROOT/followup_study/generated/tmp/dataset_filter/summary.json"',
            'python3 "$ROOT/followup_study/filter_dataset_by_score.py" \\',
...
def emit_sae_steer_script(root: Path, spec: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            f'TRAITS=(' + all_traits + ')',
            'JUDGE_CONFIGS=("$ROOT"/followup_study/generated/judge_configs/*.json)',
            'VECTOR_PATH="$ROOT/followup_study/generated/intervention_vectors/${RUN_SLUG}__sae_${SAE_LAYER}.pt"',
            'cd "$ROOT"',
            "",
            'python3 "$ROOT/followup_study/intervene_sae_features.py" \\',
            ...
            '    --max_tokens 1000 \\',
            '    --judge_config_paths "${JUDGE_CONFIGS[@]}" \\',
            '    --raw_output_path "$ROOT/followup_study/generated/raw_outputs/interventions/sae_steer/$RUN_SLUG/$eval_trait.csv" \\',
            '    --judge_output_dir "$ROOT/followup_study/generated/judge_outputs/interventions/sae_steer/$RUN_SLUG/$eval_trait" \\',
            '    --merged_output_path "$ROOT/followup_study/generated/merged_outputs/interventions/sae_steer/$RUN_SLUG/$eval_trait.csv" \\',
            '    --agreement_report_path "$ROOT/followup_study/generated/agreement_reports/interventions/sae_steer/$RUN_SLUG/$eval_trait.json" \\',
...
def emit_eval_sae_intervention_script(root: Path) -> str:
    return "\n".join(
        [
            script_header(root, root / "prev_paper_materials/persona"),
            'RUN_SLUG="${RUN_SLUG:?Set RUN_SLUG}"',
            ...
            '  --output_csv "$ROOT/followup_study/generated/intervention_reports/${RUN_SLUG}__sae.csv"',
            "",
        ]
    )
...
def emit_hallucination_feature_intervention_wrapper(root: Path, spec: dict) -> str:
    ...
    return "\n".join(
        [
            ...
            f'bash "$ROOT/followup_study/generated/runbooks/72_intervention_sae_steer.sh"',
            "",
        ]
    )
...
def main() -> None:
    ...
    runbooks = {
        ...
        "20_eval_baselines.sh": emit_eval_loop_script(repo_root, persona_root, '"$ROOT"/followup_study/generated/eval_configs/baseline/*.json'),
        "25_judge_calibration.sh": emit_calibration_script(repo_root, persona_root, spec["judge_calibration"]),
        "30_train_models.sh": emit_train_script(repo_root, persona_root, '"$ROOT"/followup_study/generated/train_configs/*.json'),
        "40_eval_finetuned_models.sh": emit_eval_loop_script(repo_root, persona_root, '"$ROOT"/followup_study/generated/eval_configs/finetuned/*.json'),
